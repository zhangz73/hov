...
df_demand = pd.read_csv("data/df_meta.csv")
df_demand = df_demand[["Date", "Hour", "HOV Travel Time", "Ordinary Travel Time"]].groupby(["Date", "Hour"]).sum().reset_index()
...
df_demand = pd.read_csv("data/df_meta.csv")
df_demand = df_demand[["Date", "Hour", "HOV Flow", "Ordinary Flow"]].groupby(["Date", "Hour"]).mean().reset_index()
df_demand["Ordinary Flow"] /= 3
