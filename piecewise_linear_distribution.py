...
if SINGLE_HOUR_RETRAIN:
    df = grid_search(rho_vals = rho_vals, toll_lst = np.arange(0, 15, 0.5), save_to_file = True, D = D, max_itr = 2000, eta = 1e-2, eps = 1e-7, min_eta = 1e-8, n_cpu = N_CPU)
else:
...
if TIME_DYNAMIC_RETRAIN:
    ...
    for t in tqdm(np.array(df_hourly_avg["Hour"].unique())):
        ...
        df = grid_search(rho_vals = rho_vals, toll_lst = np.arange(0, 15, 0.5), save_to_file = True, D = demand, max_itr = 2000, eta = 1e-2, eps = 1e-7, min_eta = 1e-8, n_cpu = N_CPU)
