...
def cost(flow, rho):
    ...
    a = 0.15
    b = 4
    return 22 * (1 + a * (flow / (CAPACITY * rho)) ** b) #r / (CAPACITY * rho - flow)
...
#            eta = eta * 0.1
#        itr += 1
#   ret = get_regime_x(float(x.detach()), tau, regime_type)
    return get_regime_x(x, tau, regime_type), loss_arr
...
#tau_lst = np.arange(0, 10, 0.5) #np.linspace(0, TAU_BAR, num = 6)[1:]
#arg_lst = list(itertools.product(rho_lst, tau_lst))
#dct = {"rho": [], "tau": [], "x1": [], "x2": [], "x3": [], "loss": [], "congestion": [], "revenue": []}
#for (rho, tau) in tqdm(arg_lst):
#    (x1, x2, x3), loss_arr = solve_regime(rho, tau, max_itr = 4000, eta = 1e0, eps = 1e-7, decay_sched = 1000, eta_min = 1e-4)
...
(x1, x2, x3), loss_arr = solve_regime(0.25, 4, max_itr = 2000, eta = 1e-1, eps = 1e-7, decay_sched = 500, eta_min = 1e-4)
print(x1, x2, x3)
#plt.plot(loss_arr)
