...
def cost(flow, rho):
    ...
    a = 0.15
    b = 4
    return 22 / 60 * (1 + a * (flow / (CAPACITY * rho)) ** b) #r / (CAPACITY * rho - flow)
...
def regime_fixedpoint_loss(x, rho, tau, regime_type):
    ...
    if regime_type == "B":
        ...
        rhs = 1 - x2
#    if regime_type in ["B", "A1"] and abs(c_delta) > 0:
#        loss = ((lhs - rhs)/c_delta) ** 2
#    elif regime_type in ["B", "A1"] and abs(c_delta) > 0:
#        loss = ((lhs - rhs)*c_delta) ** 2
#    else:
    loss = ((lhs - rhs)) ** 2
    return loss
...
for (rho, tau) in tqdm(arg_lst):
    (x1, x2, x3), regime_type, loss_arr = solve_regime(rho, tau, max_itr = 10000, eta = 1e-2, eps = 1e-7, decay_sched = 1000, eta_min = 1e-8)
...
#(x1, x2, x3), regime_type, loss_arr = solve_regime(0.25, 9, max_itr = 2000, eta = 1e-2, eps = 1e-7, decay_sched = 2000, eta_min = 1e-20)
#print(x1, x2, x3)
##print(loss_arr)
#plt.plot(loss_arr)
