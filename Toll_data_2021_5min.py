...
#os.remove('data/df_toll_5min.csv')
...
#    df_toll = df_toll.loc[df_toll['siZoneID'] == '3460 - Hesperian/238 NB']
    ...
    df_toll = df_toll[["Date", "Hour", "Minute", "Segment", "Zone_Toll"]]
    df_toll = df_toll.groupby(["Date", "Hour", "Minute", "Segment"]).mean().reset_index()
    df_toll = df_toll.rename(columns = {"Zone_Toll" : "Avg_total_toll"})
    
    with open('data/df_toll_5min.csv','a') as output_file:
        should_header = i == 0
