...
for i in range(4):
    ...
    data_filename = "data/NB_" + str(start).zfill(2) + "2021-" + str(end).zfill(2) + "2021.xlsx"
    
    df_toll = pd.read_excel(data_filename)
    ...
    df_toll = df_toll[["Date", "Hour", "Zone_Toll"]]
    df_toll = df_toll.groupby(["Date", "Hour"]).mean().reset_index()
