...
fname_lst = os.listdir("data/raw/")

if os.path.exists('data/df_PeMs_5min.csv'):
    os.remove('data/df_PeMs_5min.csv')
should_header = True
for segment in tqdm(segment_dct):
    for data_filename in tqdm(fname_lst, leave = False):
        df_flow = pd.read_csv(f"data/raw/{data_filename}", header = None, names = names)
...
#            df_flow = df_flow.dropna(axis="columns", how = 'all')
        ...
        df_flow["Minute"] = df_flow["Time"].dt.minute

        df_flow = df_flow[["Date", "Hour", "Minute", "HOV Flow", "Ordinary Flow","HOV Travel Time","Ordinary Travel Time"]]

        
        df_flow = df_flow.groupby(["Date","Hour","Minute"]).agg({"HOV Flow": "mean", "Ordinary Flow": "mean", "HOV Travel Time": "sum", "Ordinary Travel Time": "sum"}).reset_index()
        df_flow["Segment"] = segment
        df_flow["Distance"] = df_station[df_station["Segment"] == segment]["Length"].sum()
        
        with open('data/df_PeMs_5min.csv','a') as output_file:
